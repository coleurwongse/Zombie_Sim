...
def zombie_movement(x, y, x1, y1):
    ...
    if x1 < len(map):
        if map[x][y][3] == 0:
            if map[x1][y1][2] < 7:
                if map[x1][y1][0] > 5:
                    map[x1][y1][1] += int(map[x][y][1] / 4)
                    map[x][y][1] -= int(map[x][y][1] / 4)
                if map[x1][y1][1] < map[x][y][1]:
                    num = int((map[x][y][1] - map[x1][y1][1]) / 2)
                    map[x1][y1][1] += num
                    map[x][y][1] -= num
        elif map[x][y][3] == 3:
            if map[x1][y1][2] < 7:
                ...
                if map[x1][y1][1] < map[x][y][1]:
                    ...
                    map[x1][y1][1] += num
                    map[x][y][1] -= num
        elif map[x][y][3] > 0 and map[x1][y1][2] < 7:
            if map[x1][y1][0] > 5 and map[x1][y1][3] != 3:
                map[x1][y1][1] += int(map[x][y][1] / 4)
                map[i][j][1] -= int(map[x][y][1] / 4)
            elif map[x1][y1][0] > 0 and map[x1][y1][3] == 3:
                map[x1][y1][1] += int(map[x][y][1] / 4)
                map[i][j][1] -= int(map[x][y][1] / 4)
            if map[x1][y1][3] > 0 and map[x1][y1][3] != 3:
                if map[x1][y1][1] < map[x][y][1]:
                    num = int((map[x][y][1] - map[x1][y1][1]) / 2)
                    map[x1][y1][1] += num
                    map[x][y][1] -= num
            if map[x1][y1][1] < int(map[x][y][1] / 5):
                num = math.ceil((map[x][y][1] - map[x1][y1][1]) / 100)
                map[x1][y1][1] += num
                map[x][y][1] -= num
        ...
        for j in range(len(map[i])):
            if map[i][j][0] > 0:
                if map[i][j][0] < 5:
                    ...
                    if map[i][j][0] > 122 and map[i][j][4] > 0 and map[i][j][2] < 5:
                        map[i][j][2] = 5
                    if map[i][j][0] > 122 > knowledge_current and map[i][j][1] > 20 and knowledge_current - knowledge_past < 1:
                        knowledge_current += 1
                    ...
                    if map[i][j][0] > 5 and map[i][j][3] != 3:
            ...
            # Zombie movement
            if map[i][j][1] > 0:
                zombie_movement(i, j, i+1, j)
                zombie_movement(i, j, i-1, j)
                zombie_movement(i, j, i, j+1)
                zombie_movement(i, j, i, j-1)
